        ...
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                @if(!session()->has('user_token'))
                    <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-300">
                        {{ __('You must be signed in as a portal user to place calls.') }}
                    </div>
                ...
                @if(session()->has('user_token'))
                    ...
                    <form id="dialer-form" method="POST" action="{{ route('dialer.dial') }}" class="space-y-6">
                        @csrf
