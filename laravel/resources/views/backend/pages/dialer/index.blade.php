    ...
    <div class="space-y-6">
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
            <div class="p-5 space-y-6 border-t border-gray-100 dark:border-gray-800 sm:p-6">
                ...
                <div class="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
                <div class="min-w-0">
                ...
                {{-- Incoming call banner (round-robin inbound) --}}
                <div id="incoming-call-banner" class="hidden mt-6 rounded-lg border border-green-200 bg-green-50 p-5 dark:border-green-500/40 dark:bg-green-900/20">
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        <div>
                            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">{{ __('Incoming call') }}</h3>
                            <p class="text-sm text-gray-600 dark:text-gray-300">
                                {{ __('From') }} <span id="incoming-caller" class="font-semibold">—</span>
                                <span id="incoming-did" class="text-xs text-gray-500 dark:text-gray-400"></span>
                            </p>
                        </div>
                        <div class="flex items-center gap-2">
                            <button type="button" id="incoming-accept" class="btn-primary">{{ __('Accept') }}</button>
                            <button type="button" id="incoming-decline" class="btn-danger">{{ __('Decline') }}</button>
                        </div>
                    </div>
                </div>
                </div>

                <aside id="last-call-notes-panel" class="rounded-xl border border-gray-200 bg-gray-50 p-5 dark:border-gray-700 dark:bg-gray-800/50 lg:sticky lg:top-6">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h3 class="text-base font-semibold text-gray-800 dark:text-white/90">{{ __('Current call notes') }}</h3>
                            <p id="last-call-summary" class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('Start a call to add notes') }}</p>
                        </div>
                        <span id="last-call-status" class="hidden rounded-full bg-gray-200 px-2.5 py-1 text-[11px] font-semibold text-gray-700 dark:bg-gray-700 dark:text-gray-200"></span>
                    </div>
                    <textarea id="last-call-notes" rows="10" maxlength="5000" disabled
                        placeholder="{{ __('Add comments, outcome, or follow-up details…') }}"
                        class="mt-4 w-full resize-y rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-800 shadow-sm focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 disabled:cursor-not-allowed disabled:opacity-60 dark:border-gray-600 dark:bg-gray-900 dark:text-white/90"></textarea>
                    <div class="mt-3 flex items-center justify-between gap-3">
                        <span id="last-call-notes-feedback" class="text-xs text-gray-500 dark:text-gray-400"></span>
                        <button type="button" id="save-last-call-notes" class="btn-primary" disabled>{{ __('Save notes') }}</button>
                    </div>

                    <div class="mt-5 border-t border-gray-200 pt-5 dark:border-gray-700">
                        <h4 class="text-sm font-semibold text-gray-800 dark:text-white/90">{{ __('Previous call notes') }}</h4>
                        <p id="previous-call-summary" class="mt-1 text-xs text-gray-500 dark:text-gray-400">{{ __('No previous call') }}</p>
                        <div id="previous-call-notes" class="mt-3 min-h-24 whitespace-pre-wrap rounded-lg border border-gray-200 bg-white px-3 py-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200">{{ __('No notes recorded') }}</div>
                    </div>
                </aside>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const lastCallSummaryEl = document.getElementById('last-call-summary');
    const lastCallStatusEl = document.getElementById('last-call-status');
    const lastCallNotesEl = document.getElementById('last-call-notes');
    const saveLastCallNotesBtn = document.getElementById('save-last-call-notes');
    const lastCallNotesFeedbackEl = document.getElementById('last-call-notes-feedback');
    const previousCallSummaryEl = document.getElementById('previous-call-summary');
    const previousCallNotesEl = document.getElementById('previous-call-notes');
    ...
    const callSummary = (call) => {
        if (!call) return '';
        const party = call.direction === 'inbound'
            ? (call.caller_id || call.destination || 'Unknown caller')
            : (call.destination || 'Unknown destination');
        const created = call.created_at ? new Date(call.created_at).toLocaleString() : '';
        return `${call.direction || 'call'} · ${party}${created ? ` · ${created}` : ''}`;
    };

    const renderCallNotes = (latestCall, previousCall) => {
        const activeLog = callUuid && latestCall?.call_uuid === callUuid ? latestCall : null;
        const referenceCall = activeLog ? previousCall : latestCall;
        lastCallUuid = activeLog?.call_uuid || null;

        if (!activeLog) {
            if (lastCallSummaryEl) lastCallSummaryEl.textContent = callActive
                ? 'Waiting for current call log…'
                : 'Start a call to add notes';
            if (lastCallStatusEl) lastCallStatusEl.classList.add('hidden');
            if (lastCallNotesEl) {
                lastCallNotesEl.value = '';
                lastCallNotesEl.disabled = true;
            }
            if (saveLastCallNotesBtn) saveLastCallNotesBtn.disabled = true;
        } else {
            if (lastCallSummaryEl) lastCallSummaryEl.textContent = callSummary(activeLog);
            if (lastCallStatusEl) {
                lastCallStatusEl.textContent = (activeLog.status || 'unknown').replaceAll('_', ' ');
                lastCallStatusEl.classList.remove('hidden');
            }
            if (lastCallNotesEl) {
                lastCallNotesEl.value = activeLog.notes || '';
                lastCallNotesEl.disabled = false;
            }
            if (saveLastCallNotesBtn) saveLastCallNotesBtn.disabled = false;
        }

        if (previousCallSummaryEl) previousCallSummaryEl.textContent = referenceCall
            ? callSummary(referenceCall)
            : 'No previous call';
        if (previousCallNotesEl) previousCallNotesEl.textContent = referenceCall?.notes || 'No notes recorded';
    };

    const loadLastCall = async () => {
        try {
            const response = await fetch('/admin/dialer/calls/last', { headers: { 'Accept': 'application/json' } });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            const data = await response.json();
            renderCallNotes(data.call || null, data.previousCall || null);
        } catch (error) {
            console.warn('[notes] unable to load last call', error);
            if (lastCallNotesFeedbackEl) lastCallNotesFeedbackEl.textContent = 'Unable to load notes';
        }
    };

    saveLastCallNotesBtn?.addEventListener('click', async () => {
        if (!lastCallUuid || !lastCallNotesEl) return;
        saveLastCallNotesBtn.disabled = true;
        if (lastCallNotesFeedbackEl) lastCallNotesFeedbackEl.textContent = 'Saving…';
        try {
            const response = await fetch(`/admin/dialer/calls/${lastCallUuid}/notes`, {
                method: 'PUT',
                headers: {
                ...
                body: JSON.stringify({ notes: lastCallNotesEl.value })
            });
            if (!response.ok) throw new Error(`HTTP ${response.status}`);
            if (lastCallNotesFeedbackEl) lastCallNotesFeedbackEl.textContent = 'Saved';
        } catch (error) {
            if (lastCallNotesFeedbackEl) lastCallNotesFeedbackEl.textContent = 'Save failed';
        } finally {
            saveLastCallNotesBtn.disabled = false;
        }
    });
    ...
    form.addEventListener('submit', async function (event) {
        ...
        try {
            ...
            if (data.callUuid) {
                ...
                window.setTimeout(loadLastCall, 400);
