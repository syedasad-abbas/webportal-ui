...
const attachSocketFailureHandlers = (client, reject) => {
  ...
  client.on('timeout', () => {
    const err = new Error(
      `FreeSWITCH connection timed out at ${config.freeswitch.host}:${config.freeswitch.port}`
    );
    client.destroy();
    reject(err);
  });
...
const sendCommand = (command) =>
  new Promise((resolve, reject) => {
    ...
    attachSocketFailureHandlers(client, reject);
  });
...
const startAudioStream = async (uuid, websocketUrl, metadata = {}) => {
  ...
  // Keep RTP flowing while Gemini is listening or preparing a reply. Browser
  // clients otherwise interpret a quiet parked channel as lost incoming audio.
