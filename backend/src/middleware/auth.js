...
// Shared-secret guard for machine-to-machine endpoints (e.g. FreeSWITCH -> backend).
// Accepts the token via `x-internal-token` header or `token` query/body field.
const requireInternalToken = (req, res, next) => {
  const expected = config.internalTokens?.backendSync;
  if (!expected) {
